...
/**
 * Created by lanshifu on 2018/10/8.
 */

public class DemoExpandableTextViewActivity extends BaseTitleBarActivity{
    ...
    private ExpandableTextView.OnLinkClickListener linkClickListener = (type, content, selfContent) -> {
        if (type.equals(LinkType.LINK_TYPE)) {
            Toast.makeText(DemoExpandableTextViewActivity.this, "你点击了链接 内容是：" + content, Toast.LENGTH_SHORT).show();
        } else if (type.equals(LinkType.MENTION_TYPE)) {
            Toast.makeText(DemoExpandableTextViewActivity.this, "你点击了@用户 内容是：" + content, Toast.LENGTH_SHORT).show();
        } else if (type.equals(LinkType.SELF)) {
            Toast.makeText(DemoExpandableTextViewActivity.this, "你点击了自定义规则 内容是：" + content + " " + selfContent, Toast.LENGTH_SHORT).show();
        }
    };
    ...
    protected int getLayoutId() {
        return R.layout.demo_expandable_textview_activity;
    }

    @Override
    protected void initView(Bundle savedInstanceState) {

        setTitleText("扩展textview");
