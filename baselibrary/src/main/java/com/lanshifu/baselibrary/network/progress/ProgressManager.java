...
public final class ProgressManager {
    ...
    public static final String IDENTIFICATION_HEADER = "identification_header";
    public static final String LOCATION_HEADER = "Location";
    public static final int WHAT_REQUEST_FINISH = 10;
    public static final int WHAT_REQUEST_ERROR = 12;
    public static final int WHAT_RESP_FINISH = 11;
    public static final int WHAT_RESP_ERROR = 13;
    ...
    private ProgressManager() {
        ...
        this.mHandler = new Handler(Looper.getMainLooper()){
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    //请求完成或失败，移除监听
                    case WHAT_REQUEST_FINISH:
                    case WHAT_REQUEST_ERROR:
                        clearUploadLoadListener(url);
                        break;

                    //下载完成或失败，移除监听
                   case WHAT_RESP_FINISH:
                    case WHAT_RESP_ERROR:
                        clearDownLoadListener(url);
                        break;
                    default:
    ...
    private static class ProgressManagerBuilder {
        private static ProgressManager singleTon = new ProgressManager();
    }

    public static ProgressManager getInstance() {
        return ProgressManagerBuilder.singleTon;
    }
     ...
     */
    public void addUploadLoadListener(String url, ProgressListener listener) {
     ...
     * @param url
     */
    public void clearUploadLoadListener(String url) {
        synchronized (ProgressManager.class) {
            List<ProgressListener> progressListeners = mRequestListeners.get(url);
     ...
     * @param url
     */
    public void clearDownLoadListener(String url) {
        synchronized (ProgressManager.class) {
     ...
     */
    public void addDownLoadListener(String url, ProgressListener listener) {
        List<ProgressListener> progressListeners;
        synchronized (ProgressManager.class) {
    ...
    public void notifyOnErorr(String url) {
        forEachListenersOnError(mRequestListeners, url, new Exception());
        forEachListenersOnError(mResponseListeners, url, new Exception());
    }
    ...
    private Request wrapRequestBody(Request request) {
        ...
        if (mRequestListeners.containsKey(key)) {
            ...
            return request.newBuilder()
                    .method(request.method(), new ProgressRequestBody(key,mHandler, request.body(), listeners, mRefreshTime))
                    .build();
        }
        return request;
    }
    ...
    private Response wrapResponseBody(Response response) {
        ...
        if (mResponseListeners.containsKey(key)) {
            ...
            return response.newBuilder()
                    .body(new ProgressResponseBody(key,mHandler, response.body(), listeners, mRefreshTime))
                    .build();
        }
        return response;
    }
     ...
     * 请使用 {@link #addDiffResponseListenerOnSameUrl(String, ProgressListener)} 代替 {@link #addDownLoadListener}
    ...
    /**
     * 当出现需要使用同一个 {@code url} 根据 Post 请求参数的不同而下载不同资源的情况
     * 请使用 {@link #addDiffResponseListenerOnSameUrl(String, String, ProgressListener)} 代替 {@link #addDownLoadListener}
     ...
     * 请使用 {@link #addDiffRequestListenerOnSameUrl(String, ProgressListener)} 代替 {@link #addUploadLoadListener}
    ...
    public String addDiffRequestListenerOnSameUrl(String originUrl, String key, ProgressListener listener) {
        String newUrl = originUrl + IDENTIFICATION_NUMBER + key;
        addUploadLoadListener(newUrl, listener);
        return newUrl;
    }
